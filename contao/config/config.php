<?php

if (TL_MODE == 'BE') {
    $GLOBALS['TL_CSS'][] = 'system/modules/points-of-interest/assets/css/poi-be.css';
    $GLOBALS['TL_JAVASCRIPT'][] = 'system/modules/points-of-interest/assets/js/poi-be.js';
}

$GLOBALS['BE_MOD']['content']['points_of_interest'] = [
    'tables' => [
        'tl_points_of_interest',
        'tl_point_of_interest',
        'tl_content',
    ],
    'icon' => 'data:image/png;base64,',
];

$GLOBALS['TL_MODELS']['tl_points_of_interest'] = 'Oneup\Contao\Poi\Model\PointsOfInterestModel';
$GLOBALS['TL_MODELS']['tl_point_of_interest'] = 'Oneup\Contao\Poi\Model\PointOfInterestModel';

$GLOBALS['TL_CTE']['includes']['points_of_interest'] = 'Oneup\Contao\Poi\Content\Poi';

array_insert($GLOBALS['FE_MOD'], 2, array(
    'miscellaneous' => array(
        'points_of_interest' => 'Oneup\Contao\Poi\Module\Poi',
    ),
));
